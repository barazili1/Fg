...
export default function PredictionScreen({ userId, platform, subPlatform, onLogout, addToast }: PredictionScreenProps) {
  ...
  useEffect(() => {
    const multiplierMap = [1.23, 1.54, 1.93, 2.41, 4.02, 6.71, 11.18, 27.97, 69.93, 349.68];
  ...
  const triggerNextRow = () => {
    if (isScanning || scannedPath.length === 0 || activePlayRow === 10) return;
    setIsScanning(true);
    setTimeout(() => {
      setIsScanning(false);
      setActivePlayRow((prev) => (prev < 10 ? prev + 1 : prev));
    }, 450);
  };
  ...
  const handleCellClick = (rowIdx: number, colIdx: number) => {
    ...
    // Always advance activePlayRow on any cell click on the active row to keep it 100% robust and satisfying
    ...
    >
      ...
      <div className="w-full max-w-lg mx-auto flex-1 flex flex-col relative z-10 gap-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-black font-orbitron tracking-[0.2em] leading-none uppercase">
                APPLE <span className="text-crimson-bright glow-text-crimson-bright font-black">HACK</span>
              </h1>
        ...
        <div className="glass-card-premium p-5 rounded-3xl border border-white/[0.08] shadow-2xl relative flex-1 flex flex-col justify-center gap-4.5 animate-scale-up">
          ...
          {/* Interactive Game matrix - One single row centered with Odds displayed next to it */}
          <div className="flex flex-col items-center justify-center py-4 gap-4 w-full">
            {activePlayRow === 10 && (
              <div className="text-center py-2 px-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl w-full animate-bounce">
                <span className="text-xs font-black text-emerald-400">👑 تهانينا! لقد كشفت جميع التفاحات العشرة بنجاح!</span>
              </div>
            )}

            <div className="flex items-center justify-center gap-2.5 sm:gap-4 w-full p-3.5 bg-white/[0.02] border border-white/[0.05] rounded-3xl relative overflow-hidden">
              
              {/* Odds section (الاود) - small and compact next to the row */}
              <div className="flex flex-col items-center justify-center px-2 py-1 bg-amber-500/10 border border-amber-500/20 rounded-lg min-w-[58px] shadow-[0_0_8px_rgba(234,179,8,0.1)] text-center h-9 justify-center shrink-0">
                <span className="text-[6.5px] font-sans text-amber-400/90 font-black tracking-wider uppercase leading-none mb-0.5">
                  ODDS
                </span>
                <span className="text-xs font-black font-orbitron text-amber-300 glow-text-gold leading-none">
                  {scannedPath.length > 0 ? `${(activePlayRow >= 0 && activePlayRow < 10 ? [1.23, 1.54, 1.93, 2.41, 4.02, 6.71, 11.18, 27.97, 69.93, 349.68][activePlayRow] : (activePlayRow === 10 ? 349.68 : 1.23)).toFixed(2)}x` : '1.23x'}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                {Array.from({ length: 5 }).map((_, cIdx) => {
                  ...
                  if (scannedPath.length > 0) {
                    if (isSafePrediction) {
                      if (isScanning) {
                        ...
                        cellBg = 'bg-gradient-to-b from-neutral-900 to-black border-2 border-crimson shadow-[0_0_15px_rgba(239,68,68,0.25)] hover:border-crimson-bright';
                        itemNode = (
                          <motion.div
                          ...
                          >
                            ...
                            />
                            <Sparkles className="absolute top-0.5 right-0.5 w-2 h-2 text-amber-400 animate-pulse" />
                          </motion.div>
                        );
                      }
                    } else {
                  ...
                  return (
                    <button
                      key={cIdx}
                      id={`grid-cell-single-${cIdx}`}
                      onClick={() => handleCellClick(9 - gameRowIdx, cIdx)}
                      disabled={isScanning || scannedPath.length === 0 || activePlayRow === 10}
                      className={`w-11 h-11 sm:w-12 sm:h-12 rounded-full border flex items-center justify-center transition-all ${cellBg} relative p-0 overflow-hidden ${
                        scannedPath.length > 0 && activePlayRow !== 10 ? 'cursor-pointer' : 'cursor-default'
                      }`}
                    >
                      {itemNode ? (
                        itemNode
          ...
          <div className="flex gap-4 mt-1 font-sans">
            <button
              ...
              onClick={scannedPath.length > 0 && activePlayRow < 10 ? triggerNextRow : triggerScan}
              disabled={isScanning || activePlayRow === 10}
              className="flex-1 relative group overflow-hidden py-4 rounded-xl text-xs font-black tracking-widest cursor-pointer text-white disabled:opacity-50"
            >
              ...
              <span className="relative flex items-center justify-center gap-2">
                {scannedPath.length > 0 && activePlayRow < 10 ? (
                  <>
            ...
            <button
              ...
              className="px-6 rounded-xl border border-neutral-800 hover:border-crimson/50 hover:bg-crimson/10 text-slate-400 hover:text-white transition-all text-xs tracking-widest cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed font-black"
            >
